...
$('#start').on('click', function() {
    ...
    if (difficulty.length > 1) {
        $('#welcomepage').hide();
        $('#gamepage').show();
        $('#floatingLetters').empty();
        $('#divtimer div').addClass('bartimer');

        //go to first level
        onLevel();
        //gameplay style (easy/hard)
        gameStyleDifficulty();
        ...
        if (difficulty == "easy") {
                $('#divtextarea').html("Press the key of the character that's different to move onto the next level");
            } else {
